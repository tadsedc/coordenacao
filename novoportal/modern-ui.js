...
const routeMap = new Map([
  ['/ads/', '?rota=ads'],
  ['/edc/', '?rota=edc'],
  ['/professor/', '?rota=professor'],
  ['/estagio/', '?rota=estagio']
]);
...
function modernizeRoutes(root = document) {
  root.querySelectorAll('a[href]').forEach(link => {
    ...
    for (const [ending, route] of routeMap) {
      if (url.pathname.endsWith(ending)) {
        link.href = `${NOVO_ROOT}${route}`;
        break;
      }
    }
    if (link.classList.contains('loginbrand') || link.classList.contains('side-brand-link')) link.href = NOVO_ROOT;
